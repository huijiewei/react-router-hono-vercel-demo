...
import { type RouteManifest, type RouteManifestEntry, routeManifestToRouteConfig } from "./manifest";
...
export default async function flexRoutes(options: FolderRoutesOptions = {}) {
  ...
  for (const file of files) {
    const normalizedFile = normalizePath(join(routesDirectory, file));
